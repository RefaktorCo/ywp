...
-   Update Angular component peer dependencies to include Angular 12.
...
-   `<doka-image-editor>` custom element has been renamed to `<pintura-editor>`.
-   `<image-input>` has been renamed to `<pintura-input>`
-   The `.doka-image-editor` CSS class has been renamed to `.pintura-editor`.
-   `'doka:<event>'` has been renamed to `'pintura:<event>'`.
-   `doka-` internal id prefixes have been removed.
-   All other use of the term "doka" has been replaced with "pintura".
-   Cordova backup script now stores backup WebAPI in `__pqina_webapi__`.
-   Removed method `undo()`, use `history.undo()` instead.
-   Removed method `redo()`, use `history.redo()` instead.
-   Removed method `revert()`, use `history.revert()` instead.
-   Removed export `imageOrienter`, use `createDefaultImageOrienter`.
-   Removed export `plugin_annotate_defaults`, use `markup_editor_defaults` instead.
-   Removed export `plugin_decorate_defaults`, use `markup_editor_defaults` instead.
-   Removed export `component_shape_editor_locale_en_gb`, use
    `markup_editor_locale_en_gb`
-   Removed export `plugin_crop_defaults`, set `cropSelectPresetOptions` manually instead, see crop plugins docs.
-   Removed export `plugin_resize_defaults`, this was an empty object, no replacement necessary.
-   Removed export `plugin_sticker_defaults`, this was an empty object, no replacement necessary.

## 7.18.3 | 2021-06-20

-   Detect and correct missing mimetype in DataURLs.
...
-   Update Svelte preset with FilePond example.
-   Update FilePond Plugin Image Editor to remove the edit button if WebGL is disabled in the browser.
-   Update FilePond Plugin Image Editor to still auto-process images if editing is disabled by lack of WebGL support.
